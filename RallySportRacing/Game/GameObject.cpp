...
namespace Game {
	...
	void GameObject::translate(glm::vec3 speedVector) {
		position = bulletToGlm(rigidBody->getWorldTransform().getOrigin());
	...
	void GameObject::setupRigidbody() {
		...
		btRigidBody::btRigidBodyConstructionInfo groundRigidBodyCI(1, motionState, collisionShape, btVector3(0, 0, 0));
	...
	btVector3 Game::GameObject::glmToBullet(const glm::vec3& vec)
	{
		return { vec.x, vec.y, vec.z };
	}
	btQuaternion Game::GameObject::glmToBullet(const glm::quat& q)
	{
		return btQuaternion(q.x, q.y, q.z, q.w);
	}
	glm::vec3 Game::GameObject::bulletToGlm(const btVector3& v)
	{
		return glm::vec3(v.getX(), v.getY(), v.getZ());
	}
	glm::quat Game::GameObject::bulletToGlm(const btQuaternion& q)
	{
